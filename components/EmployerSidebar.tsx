...
import { EmployerView } from '../types';
import { DashboardIcon, PlusCircleIcon, UsersIcon, LogoIcon, ChartBarIcon } from './icons/Icons';
...
const EmployerSidebar: React.FC<EmployerSidebarProps> = ({ currentView, setView }) => {
  // FIX: Replaced JSX.Element with React.ReactElement to resolve the 'Cannot find namespace JSX' error.
  const navItems: { view: EmployerView; label: string; icon: React.ReactElement }[] = [
    { view: 'dashboard', label: 'Dashboard', icon: <DashboardIcon /> },
    { view: 'postJob', label: 'Post a Job', icon: <PlusCircleIcon /> },
    { view: 'analytics', label: 'Analytics', icon: <ChartBarIcon /> },
    { view: 'candidates', label: 'View Candidates', icon: <UsersIcon /> },
  ];
    ...
    <div className="w-64 bg-white border-r border-slate-200 flex-col hidden lg:flex">
          ...
          {navItems.map((item) => (
            <li key={item.view}>
              <button
                ...
                // Note: 'candidates' view is accessed via dashboard, so we disable direct nav for now
                disabled={item.view === 'candidates'}
              >
      ...
      <div className="p-4 border-t border-slate-200">
          <div className="p-4 rounded-lg bg-slate-100 text-center">
              <h3 className="font-bold text-slate-800">Enterprise Plan</h3>
              <p className="text-sm text-slate-600 mt-1">Unlock AI recruiting and advanced analytics.</p>
